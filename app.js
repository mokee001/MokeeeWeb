...
class DistortionManager {
  ...
  update(uniforms) {
    ...
    uniforms.baseScale.value = CONFIG.distortion.baseScale;
  }
}
...
class App {
  constructor() {
    ...
    // Animate intro distortion (flat -> curved), delayed 200ms after blur animation starts
    this.distortionManager.intensity = CONFIG.distortion.flat;
    this.distortionManager.targetIntensity = CONFIG.distortion.flat;

    // Hide loader
    requestAnimationFrame(() => {
      document.getElementById('loader').classList.add('hidden');
      // Trigger entrance animation (blur fade starts now)
      requestAnimationFrame(() => {
        document.body.classList.remove('intro');
        // Delay distortion animation by 200ms after blur animation starts
        setTimeout(() => {
          this.distortionManager.setDistortion(CONFIG.distortion.curved);
        }, 270);
      });
    });
  ...
  animate = () => {
    ...
    const intersects = this.navigation.isDragging ? [] : this.raycaster.intersectObjects(hitMeshes);
